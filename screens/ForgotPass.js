...
const ForgotPassScreen = ({ navigation }) => {
  const { LoginUser, datal, setDatal,PasswordReset,setDataF,dataF } = useContext(EcomContext);

  const { colors } = useTheme();

  const a = () => {
    if (datal.Signupl) {
   //   navigation.navigate("StartStackScreen");
    ...
    {/*     <TouchableOpacity
        ...
        <View style={styles.button}>
          <TouchableOpacity
            style={styles.signIn}
            onPress={() => PasswordReset()}
            //   onPress={() => navigation.navigate("StartStackScreen")}
          >
            <LinearGradient
